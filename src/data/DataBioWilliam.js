import ImageOne from '../images/wlfase1.jpg';
import ImageTwo from '../images/wlfase2.jpg';
import ImageThree from '../images/wlfase3.jpg';
import ImageFour from '../images/wlfase4.jpg';
import ImageFive from '../images/wlfase5.jpg';
import ImageSix from '../images/g-jovem.jpg';
import ImageSeven from '../images/g-adulto.jpg';
import ImageEight from '../images/supertyrant.jpg';
import ImageNine from '../images/lck.jpg';
import ImageTen from '../images/ivys2.jpg';
import ImageEleven from '../images/cerberus.jpg';
import ImageTwelve from '../images/alligator.jpg';


export const BioWilliamContent = {
    title1: "Rostos conhecidos",
    paragraph1: "Marvin Branagh era um dos policiais que integravam a delegacia de Polícia de Raccoon City. Mesmo com a cidade sob lei marcial Marvin mantinha suas funções normais e até preparou a festa de boas vindas para Leon S. Kennedy porém, algo saiu errado.",
    img1: ImageOne,
    alt1: "Marvin ferido",

    title2: "Corruptos na delegacia",
    paragraph2: "Brian Irons era o chefe do departamento de Polícia de Raccoon City. Ao longo dos anos que esteve à frente do comando da polícia, Irons passou a imagem de um líder dedicado e preocupado com a segurança dos cidadãos de R.C.",
    img2: ImageTwo,
    alt2: "Claire Delegacia",

    title3: "Responsável de tudo",
    paragraph3: "Dr. William Birkin foi um virologista que trabalhou para a Umbrella Farmaceuticas entre as décadas de 1970-1990. Um dos principais cientistas por trás do aperfeiçoamento do T-Vírus e com suas pesquisas desenvolveu o G-Vírus.",
    img3: ImageThree,
    alt3: "Birkin Mutação",

    title4: "Zumbis mais rápidos e resistentes",
    paragraph4: "Os inimigos do game estão mais inteligentes e resistentes, mas também é possível atirar em pontos vitais para retarda-los ou o famoso stagger para poder passar livremente nos ambientes sem ser atacado.",
    img4: ImageFour,
    alt4: "Rosto de zumbi",

    title5: "Inimigos Clássicos e assustadores",
    paragraph5: "Os Vilões clássicos da série estão de volta com tudo, e vão caçar você do começo ao fim do game, não de bobeira ainda mais se for em uma dificuldade Insana, corra ou enfrente esses terríveis inimigos.",
    img5: ImageFive,
    alt5: "licker",

    title6: "Cenários de chamar à atenção",
    paragraph6: "Assim como as ruas e delegacia, os esgotos de Raccon City estão lindos e concerteza quem jogou o clássico de 1998 vai se surpreender com a maravilha que está esse cenário e os inimigos que habitam nele.",
    img6: ImageSix,
    alt6: "Leon no esgoto",

    title7: "Rostos conhecidos",
    paragraph7: "Marvin Branagh era um dos policiais que integravam a delegacia de Polícia de Raccoon City. Mesmo com a cidade sob lei marcial Marvin mantinha suas funções normais e até preparou a festa de boas vindas para Leon S. Kennedy porém, algo saiu errado.",
    img7: ImageSeven,
    alt7: "Marvin ferido",

    title8: "Rostos conhecidos",
    paragraph8: "Marvin Branagh era um dos policiais que integravam a delegacia de Polícia de Raccoon City. Mesmo com a cidade sob lei marcial Marvin mantinha suas funções normais e até preparou a festa de boas vindas para Leon S. Kennedy porém, algo saiu errado.",
    img8: ImageEight,
    alt8: "Marvin ferido",

    title9: "Rostos conhecidos",
    paragraph9: "Marvin Branagh era um dos policiais que integravam a delegacia de Polícia de Raccoon City. Mesmo com a cidade sob lei marcial Marvin mantinha suas funções normais e até preparou a festa de boas vindas para Leon S. Kennedy porém, algo saiu errado.",
    img9: ImageNine,
    alt9: "Marvin ferido",

    title10: "Rostos conhecidos",
    paragraph10: "Marvin Branagh era um dos policiais que integravam a delegacia de Polícia de Raccoon City. Mesmo com a cidade sob lei marcial Marvin mantinha suas funções normais e até preparou a festa de boas vindas para Leon S. Kennedy porém, algo saiu errado.",
    img10: ImageTen,
    alt10: "Marvin ferido",

    title11: "Rostos conhecidos",
    paragraph11: "Marvin Branagh era um dos policiais que integravam a delegacia de Polícia de Raccoon City. Mesmo com a cidade sob lei marcial Marvin mantinha suas funções normais e até preparou a festa de boas vindas para Leon S. Kennedy porém, algo saiu errado.",
    img11: ImageEleven,
    alt11: "Marvin ferido",

    title12: "Rostos conhecidos",
    paragraph12: "Marvin Branagh era um dos policiais que integravam a delegacia de Polícia de Raccoon City. Mesmo com a cidade sob lei marcial Marvin mantinha suas funções normais e até preparou a festa de boas vindas para Leon S. Kennedy porém, algo saiu errado.",
    img12: ImageTwelve,
    alt12: "Marvin ferido",
    
}
